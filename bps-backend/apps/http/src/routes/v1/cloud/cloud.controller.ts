...
import { deleteFile, get_parsed_error_message, getRejectionReason, uploadFile } from "../utils/helper.js";
...
export const upload_image = async (req: Request, res: Response, next: NextFunction) => {
    ...
    if(!uploaded_file){
        res.status(HTTP_STATUS.BAD_REQUEST).json({"error": "no file uploaded", reason: await getRejectionReason()});
        return;
    }
